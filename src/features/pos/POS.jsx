    ...
    <div className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="relative flex-1">
              <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-black/20 px-3 py-3">
                ...
                <input ref={productInputRef} id="pos-query" name="pos-query" autoComplete="off" value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm outline-none placeholder:text-white/35" placeholder="Escanea codigo de barras, SKU, IMEI o busca producto" aria-label="pos-query" />
              </div>
              {filtered.length > 0 && (
            ...
            <select id="pos-ncf-type" name="pos-ncf-type" autoComplete="off" value={ncfType} onChange={(event) => changeNcfType(event.target.value)} className="rounded-lg border border-white/10 bg-[#111118] px-3 py-3 text-sm font-bold outline-none" aria-label="pos-ncf-type">
            ...
            <select id="pos-mode" name="pos-mode" autoComplete="off" value={mode} onChange={(event) => setMode(event.target.value)} className="rounded-lg border border-white/10 bg-[#111118] px-3 py-3 text-sm font-bold outline-none" aria-label="pos-mode">
      ...
      <section className="panel min-w-0 rounded-lg p-4">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <select id="pos-payment-method" name="pos-payment-method" autoComplete="off" value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)} className="rounded-lg border border-white/10 bg-[#0d0e14] px-3 py-3 text-sm outline-none" aria-label="pos-payment-method">
            ...
            <div key={`${item.productId}-${item.serials?.join('-') || 'bulk'}`} className="rounded-lg border border-white/10 bg-white/[0.035] p-3">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                <label>
                  ...
                  <input type="number" min="0" step="0.01" autoComplete="off" id={"pos-item-price-" + index} name={"pos-item-price-" + index} value={item.price} onChange={(event) => updateCartLine(item, { price: Number(event.target.value) })} className="input-dark py-2" />
                </label>
                <label>
                  <span className="label-dark">Rebaja %</span>
                  <input type="number" min="0" max="10" step="0.01" autoComplete="off" id={"pos-item-discount-" + index} name={"pos-item-discount-" + index} value={item.discount || 0} onChange={(event) => updateCartLine(item, { discount: Math.min(Math.max(Number(event.target.value), 0), 10) })} className="input-dark py-2" />
                </label>
              </div>
            </div>
          ))}
      ...
      <Modal open={customerModal} onClose={() => setCustomerModal(false)} title="Registrar cliente" size="md" footer={<div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setCustomerModal(false)}>Cancelar</Button><Button variant="success" onClick={saveQuickCustomer}>Guardar cliente</Button></div>}>
        <div className="grid gap-3 md:grid-cols-2">
          <label><span className="label-dark">Tipo</span><select id="pos-customer-type" name="pos-customer-type" autoComplete="off" value={customerDraft.type} onChange={(event) => setCustomerDraft((state) => ({ ...state, type: event.target.value }))} className="input-dark"><option value="persona">Persona</option><option value="empresa">Empresa</option><option value="final">Final</option></select></label>
          <label><span className="label-dark">Nombre</span><input id="pos-customer-name" name="pos-customer-name" autoComplete="off" value={customerDraft.name} onChange={(event) => setCustomerDraft((state) => ({ ...state, name: event.target.value }))} className="input-dark" autoFocus /></label>
          <label><span className="label-dark">RNC / Cedula</span><input id="pos-customer-document" name="pos-customer-document" autoComplete="off" value={customerDraft.document} onChange={(event) => setCustomerDraft((state) => ({ ...state, document: event.target.value }))} className="input-dark" /></label>
          <label><span className="label-dark">Telefono</span><input id="pos-customer-phone" name="pos-customer-phone" autoComplete="tel" value={customerDraft.phone} onChange={(event) => setCustomerDraft((state) => ({ ...state, phone: event.target.value, whatsapp: event.target.value }))} className="input-dark" /></label>
        </div>
      </Modal>
    </div>
  )
