    ...
    <div className="flex h-full gap-5">
      ...
      <Modal open={customerModal} onClose={() => setCustomerModal(false)} title="Cliente rapido" size="md" footer={<div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setCustomerModal(false)}>Cancelar</Button><Button variant="success" onClick={saveQuickCustomer}>Guardar</Button></div>}>
        <div className="grid gap-3 md:grid-cols-2">
          <label htmlFor="quote-quick-customer-type"><span className="label-dark">Tipo</span><select id="quote-quick-customer-type" name="quickCustomerType" value={customerDraft.type} onChange={(e) => setCustomerDraft((s) => ({ ...s, type: e.target.value }))} className="input-dark"><option value="persona">Persona</option><option value="empresa">Empresa</option><option value="final">Final</option></select></label>
          <label htmlFor="quote-quick-customer-name"><span className="label-dark">Nombre</span><input id="quote-quick-customer-name" name="quickCustomerName" value={customerDraft.name} onChange={(e) => setCustomerDraft((s) => ({ ...s, name: e.target.value }))} className="input-dark" autoFocus /></label>
          <label htmlFor="quote-quick-customer-document"><span className="label-dark">RNC / Cedula</span><input id="quote-quick-customer-document" name="quickCustomerDocument" value={customerDraft.document} onChange={(e) => setCustomerDraft((s) => ({ ...s, document: e.target.value }))} className="input-dark" /></label>
          <label htmlFor="quote-quick-customer-phone"><span className="label-dark">Telefono</span><input id="quote-quick-customer-phone" name="quickCustomerPhone" value={customerDraft.phone} onChange={(e) => setCustomerDraft((s) => ({ ...s, phone: e.target.value, whatsapp: e.target.value }))} className="input-dark" /></label>
        </div>
      </Modal>
