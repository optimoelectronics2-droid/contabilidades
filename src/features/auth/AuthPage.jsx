    ...
    <main className="grid min-h-screen place-items-center bg-[#0A0A0F] p-4 text-white">
      <form onSubmit={submit} className="panel w-full max-w-md rounded-lg p-6">
        ...
        <div className="space-y-3">
          {mode === 'register' ? <Input id="auth-name" name="name" label="Nombre" autoComplete="name" value={form.name} onChange={(value) => setForm((state) => ({ ...state, name: value }))} /> : null}
          <Input id="auth-email" name="email" label="Email" autoComplete="email" value={form.email} onChange={(value) => setForm((state) => ({ ...state, email: value }))} />
          <Input id="auth-password" name="password" label="Contraseña" type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} value={form.password} onChange={(value) => setForm((state) => ({ ...state, password: value }))} />
        </div>
...
function Input({ id, name, label, value, onChange, type = 'text', autoComplete }) {
  return <label htmlFor={id}><span className="label-dark">{label}</span><input id={id} name={name || id} type={type} value={value} autoComplete={autoComplete} onChange={(event) => onChange(event.target.value)} className="input-dark" /></label>
}
